package dy.whatsong.global.jwt.service;
...
public class JwtService {
    ...
    public String createAccessToken(String email) {
        ...
        return JWT.create() // JWT 토큰을 생성하는 빌더 반환
                ...
                .withExpiresAt(new Date(now.getTime() + jwtProperties.getACCESS_TOKEN_EXPIRED_TIME())) // 토큰 만료 시간 설정

                //클레임으로는 저희는 email 하나만 사용합니다.
                //추가적으로 식별자나, 이름 등의 정보를 더 추가하셔도 됩니다.
                //추가하실 경우 .withClaim(클래임 이름, 클래임 값) 으로 설정해주시면 됩니다
                .withClaim(EMAIL_CLAIM, email)
                .sign(Algorithm.HMAC512(jwtProperties.getJWT_SECRET_KEY())); // HMAC512 알고리즘 사용, application-jwt.yml에서 지정한 secret 키로 암호화
    }
    ...
    public String createRefreshToken() {
        ...
        return JWT.create()
                ...
                .withExpiresAt(new Date(now.getTime() + jwtProperties.getREFRESH_TOKEN_EXPIRED_TIME()))
    ...
    public void sendAccessToken(HttpServletResponse response, String accessToken) {
        ...
        response.setHeader(jwtProperties.getACCESS_TOKEN_HEADER(), accessToken);
    ...
    public void setRefreshTokenHeader(HttpServletResponse response, String refreshToken) {
        response.setHeader(jwtProperties.getREFRESH_TOKEN_HEADER(), refreshToken);
    }
